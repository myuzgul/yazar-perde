...
ensureDir(path.join('src', 'components', 'admin'));
    ...
    <aside className="w-full lg:w-64 bg-slate-900 text-slate-300 flex flex-col shrink-0 min-h-screen border-r border-slate-800">
      <div className="p-5 border-b border-slate-800 flex items-center justify-between">
        <Link href="/panel" className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#1B84F8] text-white flex items-center justify-center font-bold shadow-md shadow-[#1B84F8]/20">
            YP
          </div>
          <div>
            <h2 className="text-sm font-bold text-white tracking-wider">YAZAR PERDE</h2>
            <span className="text-[10px] text-[#1B84F8] font-semibold bg-[#1B84F8]/10 px-1.5 py-0.5 rounded border border-[#1B84F8]/20">
              YÖNETİM v1.0
            </span>
          </div>
        </Link>
...
`;
fs.writeFileSync(path.join('src', 'components', 'admin', 'AdminSidebar.tsx'), sidebarContent);

// 2. src/app/(admin)/panel/katsayilar/page.tsx
ensureDir(path.join('src', 'app', '(admin)', 'panel', 'katsayilar'));
const katsayilarContent = `'use client';
...
`;
fs.writeFileSync(path.join('src', 'app', '(admin)', 'panel', 'katsayilar', 'page.tsx'), katsayilarContent);

// 3. src/app/(admin)/panel/ayarlar/page.tsx
ensureDir(path.join('src', 'app', '(admin)', 'panel', 'ayarlar'));
const ayarlarContent = `'use client';

import React, { useState, useEffect } from 'react';
import AdminSidebar from '@/components/admin/AdminSidebar';
import { Settings, Save, CheckCircle2, AlertCircle, RefreshCw, Phone, Globe, Tag, Truck } from 'lucide-react';
...
`;
fs.writeFileSync(path.join('src', 'app', '(admin)', 'panel', 'ayarlar', 'page.tsx'), ayarlarContent);

console.log('All Admin Panel pages generated successfully.');
